...
class FirebaseCalendarRepository extends CalendarRepository {
  ...
  Future<void> addEvent(Station station, DateTime from, DateTime to,
    ...
    await userRef.doc(docRef.id).set({
      "eventRef": FirebaseFirestore.instance
          .doc('computerStations/${station.id}/events/${docRef.id}'),
      "to": to,
      "from": from,
    });
  }
  ...
  Future<void> removeEvent(
      Station station, Booking booking, String userId) async {
    CollectionReference bookingRef = FirebaseFirestore.instance
        .collection('computerStations')
        .doc(station.id)
        .collection('events');
  ...
  Future<List<Booking>> getMyEventsByDay(
      String userId, String stationId, DateTime dayToCheck) async {
    QuerySnapshot bookings = await FirebaseFirestore.instance
        ...
        .where('to',
            isGreaterThanOrEqualTo:
                Timestamp.fromDate(getMorningOfDay(dayToCheck)),
            isLessThanOrEqualTo: Timestamp.fromDate(getEndOfDay(dayToCheck)))
        .get();
